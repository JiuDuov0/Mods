            ...
            <el-row :gutter="20">
                ...
                <el-col :span="19">
                    ...
                    <el-card v-if="ModDependenceEntities.length > 0" style="margin-top: 20px;">
                        <h3>前置依赖</h3>
                        <ul>
                            <li v-for="dependence in ModDependenceEntities" :key="dependence.ModDependenceId" @click="goToModDetail(dependence.DependenceModVersion.Mod.ModId)">
    ...
    export default {
        ...
        methods: {
            modDetail() {
                ...
                $.ajax({
                    url: 'https://127.0.0.1:7114/api/Mod/GetModPointByModId',
                    type: "POST",
                    contentType: "application/json; charset=utf-8",
                    headers: {
                        'Authorization': 'Bearer ' + localStorage.getItem('token')
            ...
            addPoint() {
                $.ajax({
                    url: 'https://127.0.0.1:7114/api/Mod/AddModPoint',
                    type: "POST",
                    contentType: "application/json; charset=utf-8",
                    headers: {
                        'Authorization': 'Bearer ' + localStorage.getItem('token')
            ...
            updatePoint() {
                $.ajax({
                    url: 'https://127.0.0.1:7114/api/Mod/UpdateModPoint',
                    type: "POST",
                    contentType: "application/json; charset=utf-8",
                    headers: {
                        'Authorization': 'Bearer ' + localStorage.getItem('token')
            ...
            async handleDownload(FileId, VersionNumber) {
                ...
                try {
                    const response = await this.$axios({
                        url: 'https://127.0.0.1:7114/api/Files/DownloadFile',
                        method: 'POST',
                        data: { FileId: FileId },
                        responseType: 'blob',
